...
fn main() {
  ...
  let heroku_api_token = env::var("HK_API_TOKEN").expect("HK_API_TOKEN env variable not found in the .env");
  let mut client = platform_api::PlatformAPI::new(heroku_api_token.to_string());
  let heroku_config: Config = serde_yaml::from_str(&yaml_file).expect("The configuration file is not in the expected format.");

  for app in heroku_config.apps {
    println!("Updating app {}", app.name);
    println!("With settings => {:?}", app.settings);
    if app.settings.is_empty() {
      println!("Skipping update for => {:?}, no settings were found.", app.name);
    }
    else {
...
fn read_config_file() -> String {
  ...
  let mut f = File::open(complete_path).expect("Unable to open config.yml file. Please create the file in ~/.heroku-env/");
  f.read_to_string(&mut data).expect("Unable to read the config.yml file.");
  data
}
